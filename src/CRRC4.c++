...
CRRC4::CRRC4(string Name, int Label, int Coder, TFile *file, double TOTAL_TIME) : Detector(Name, Label, Coder, file, TOTAL_TIME)
{
    ...
    ChannelPileUp = new TH2D((Name + "_ChannelPileUp").c_str(), (Name + "_ChannelPileUp").c_str(), TOTAL_TIME/100, 0, TOTAL_TIME, BIN, MIN, MAX);
    ChannelSaturated = new TH2D((Name + "_ChannelSaturated").c_str(), (Name + "_ChannelSaturated").c_str(), TOTAL_TIME/100, 0, TOTAL_TIME, BIN, MIN, MAX);
}
